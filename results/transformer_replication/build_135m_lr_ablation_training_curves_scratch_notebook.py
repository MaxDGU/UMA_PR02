#!/usr/bin/env python3
"""
Build and execute the scratch 135M LR ablation notebook with epochwise SP2013 profiles.
"""
...
def main() -> None:
    ...
    out = root / "results" / "transformer_replication" / "analyze_135m_lr_ablation_training_curves_scratch.ipynb"
    ...
    cells.append(
        nbf.v4.new_markdown_cell(
            dedent(
                r"""
                # 135M Scratch LR Ablation: Epochwise Training and SP2013 Accuracy Profiles

                **Question.** How do the scratch-trained `SmolLM2-135M` learning-rate ablation runs change SP2013 performance across the epochs completed so far, and where do those changes concentrate in the `ED/UD × add/sub/mul/div` profile?

                **Setup.** The notebook reads the scratch `135M` run directories under
                `results/transformer_replication/smol2_135m_*_scratch_matchspval_iduma_*`.
                These runs correspond to the `s135scr_*` jobs launched on `2026-04-12 16:29:41`.

                **Objects.** For run \(r\), completed validation epoch \(e\), denominator class
                \(d \in \{\mathrm{ED}, \mathrm{UD}\}\), and operation
                \(o \in \{\mathrm{add}, \mathrm{sub}, \mathrm{mul}, \mathrm{div}\}\), we study
                \[
        ...
        nbf.v4.new_markdown_cell(
            dedent(
                ...
                **Protocol.** Every run uses the same translated synthetic corpus and the same held-out SP2013
                validation condition. The comparison varies only learning rate and scheduler.

                **Artifact interpretation.** The notebook separates:

                1. **Optimization view.** Training loss against epoch progress, including any later partial epoch that
                   appears in `last/history.json`.
                2. **Evaluation view.** SP2013 overall and cellwise accuracy using only completed full validation
                   artifacts currently written to disk.

                **Important detail.** The training-loss view can extend beyond the cellwise profile figures whenever a
                run has logged later history checkpoints than completed `id_val_epoch_XX.csv` artifacts. The run
                availability table below reports the current per-run state directly from disk.
                """
            )
        )
    )
        ...
        nbf.v4.new_code_cell(
            dedent(
                ...
                RUN_NAME_RE = re.compile(
                    r"lr(?P<lr_token>[0-9]+em[0-9]+)_(?P<scheduler_token>cos|lin)_e5_20260412_162941$"
                )
                ...
                run_dirs = sorted(
                    RUN_DIR.glob(
                        "smol2_135m_synth_unique_seed1_all1000_mincols_paramsalways_scratch_matchspval_iduma_lr*_e5_20260412_162941"
                    )
                )
                if not run_dirs:
                    raise FileNotFoundError("No scratch 135M LR ablation run directories were found.")

                display(Markdown(f"Detected **{len(run_dirs)}** scratch LR-ablation runs with current on-disk artifacts."))
                """
            )
        )
    )
    ...
    cells.append(
        nbf.v4.new_code_cell(
            dedent(
                ...
                fig1.suptitle("135M Scratch LR Ablation: Optimization and Overall Accuracy by Epoch", fontsize=18)
                ...
                fig2.suptitle("135M Scratch LR Ablation: Mean Absolute Gap by Epoch", fontsize=18)
                ...
                if SAVE_FIGURE:
                    for suffix in ["png", "pdf"]:
                        fig1.savefig(FIGURE_DIR / f"135m_lr_ablation_scratch_epochwise_summary.{suffix}", bbox_inches="tight")
                        fig2.savefig(FIGURE_DIR / f"135m_lr_ablation_scratch_epochwise_mag.{suffix}", bbox_inches="tight")

                fig3 = plot_epoch_profile_curves(
                    title="135M Scratch LR Ablation: SP2013 True-Answer Profile by Completed Epoch",
                    file_stem="135m_lr_ablation_scratch_epochwise_true_curve_profiles",
                )
                fig4 = plot_cell_trajectories(
                    value_col="true_accuracy_pct",
                    title="135M Scratch LR Ablation: Cellwise True-Answer Accuracy by Completed Epoch",
                    file_stem="135m_lr_ablation_scratch_epochwise_true_profiles",
                    include_refs=True,
                )
                fig5 = plot_cell_trajectories(
                    value_col="response_accuracy_pct",
                    title="135M Scratch LR Ablation: Cellwise Accuracy Against UMA Target by Completed Epoch",
                    file_stem="135m_lr_ablation_scratch_epochwise_response_profiles",
                    include_refs=False,
                )
    ...
    cells.append(
        nbf.v4.new_markdown_cell(
            dedent(
                ...
                **Finding.** The notebook now includes two complementary epochwise profile views. The new multi-panel
                curve figure keeps the original 8-cell profile style and shows one full profile per completed epoch. The
                2×4 cell panels still show each `ED/UD × operation` cell as its own trajectory.

                **Interpretation.** MAG compresses the same 8-cell true-answer profile to a single number relative to
                the Human and UMA reference curves; lower is better. In the `true_mag_human_pct` panel, the dashed
                horizontal line is the fixed UMA-to-Human gap, so student curves below that line are closer to the
                Human profile than UMA is. If a scratch run has only one or two points in the profile views, that does
                not mean the run stopped. It means the next full `id_val_epoch_XX.csv` artifact has not been written yet.

                **Use.** This notebook is the right place to revisit once the scratch continuation jobs write
                `id_val_epoch_02.csv` or later artifacts; the same plotting code will automatically extend the lines.
                """
            )
        )
    )
