...
├── rollouts/                                 # consolidated raw rollouts (one .csv.gz per size)
│   ├── rollouts_0p6B.csv.gz                  # stage,prob,op,gt,rollout_idx,T,raw_text,pred,correct
│   ├── rollouts_1p7B.csv.gz
│   ├── rollouts_4B.csv.gz                    # 5 stages (no humanFT ep2)
│   └── rollouts_8B.csv.gz
└── {0p6B,1p7B,4B,8B}/
