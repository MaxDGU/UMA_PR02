...
This folder is a compact share bundle for the cleaned FoundationalASSIST dataset.

## Contents

- `problems.csv`: one row per cleaned problem.
- `student_answers.csv.gz`: raw row-level student `answer_text` from FoundationalASSIST interactions for the problem ids, with source student ids replaced by `student_hash`.
...
`problems.csv` preserves problem text, answer key, domain family, split, skill labels, interaction counts, and top answer surfaces.
